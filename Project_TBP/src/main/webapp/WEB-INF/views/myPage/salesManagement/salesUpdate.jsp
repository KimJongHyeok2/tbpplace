	...
	success: function(data, status) {
		if(status == "success") {
			...
			var category = data.list;
 			var state = "<ul>";
			for(var i=0; i<count; i++) {
 				state += "<li value='" + category[i].productStateId + "'><span id='state" + category[i].productStateId + "'>" + category[i].productStateName + "</span></li>";
			}
 			state += "</ul>";
...
$("#testimg1").val("${pageContext.request.contextPath}/resources/upload/xboxone.jpg");
// 상품상태 선택시
...
// 등록완료시
 $("#registerOk").click(function() {
	...
	if(categoryValidity && wayValidity && stateValidity && titleValidity && contentValidity && priceValidity && stockValidity && fileList.length != 0 && mainThumbnailValidity) {
		$.ajax({
			url: getContextPath() + "/product/update",
			type: "POST",
			cache: false,
			data: {
			...
			success: function(data, status) {
				if(status == "success") {
					if(data == "Ok") {
						...
						$("#registerPanel").css("display", "block");
						$("#registerText").html("상품 수정에 실패하였습니다.");
						$("#tbpRegisterCheckBtn").attr("onclick", "$('#registerPanel').css('display', 'none')");
					}
				}
			}
...
function updateSalesWay() {
	var salesWayId = ${productDto.salesWayId};
	wayValidity = true;
	$("#way_Category").css("display", "none");
	$("#way_checkIMG").attr("src", getContextPath() + "/resources/img/join/after_all.png");
	$("#way_inputOkIMG").attr("src", getContextPath() + "/resources/img/join/after_all.png");
	if(salesWayId == 1) {		
		$("#way_Select").html("직접거래");
	} else {
		$("#way_Select").html("택배거래");		
	}
	if(salesWayId == 1) {
		wayValidity = true;
		$("#way_Box2").css("display", "inline-block");
		$("#way_inputOkIMG").attr("src", getContextPath() + "/resources/img/join/after_all.png");
		$("#selectWayId").val(salesWayId);
	} else {
		wayValidity = true;
		$("#way_Box2").css("display", "none");
		$("#productAddr").val("");
...
function updateState() {
	var stateId = ${productDto.productStateId};
	stateValidity = true;
	$("#state_Category").css("display", "none");
	$("#state_checkIMG").attr("src", getContextPath() + "/resources/img/join/after_all.png");
	$("#state_inputOkIMG").attr("src", getContextPath() + "/resources/img/join/after_all.png");
	if(stateId == 1) {
		$("#state_Select").html("미개봉 새제품");		
	} else if(stateId == 2) {
		$("#state_Select").html("새제품");	
	} else if(stateId == 3) {
		$("#state_Select").html("중고");		
	} else if(stateId == 4) {
		$("#state_Select").html("중고(하자있음)");	
	} else {
		$("#state_Select").html("중고(하자심함)");			
	}
	$("#selectStateId").val(stateId);
}
function updateCategory() {
	var categoryId = ${productDto.categoryId};
	categoryValidity = true;
	$.ajax({
		url: "${pageContext.request.contextPath}/category/updateDepth",
		type: "POST",
		cache: false,
		data: {
		...
		success: function(data, status) {
			if(status == "success") {
				for(var i=0; i<data.length; i++) {
					if(i==0) {						
						$("#depth3_Select").html(data[i]);
					} else if(i==1) {						
						$("#depth2_Select").html(data[i]);
					} else {						
						$("#depth1_Select").html(data[i]);
					}
				}
			}
		}
...
<div class="titleBox">
	<div class="mainTitle">상품수정</div>
	<div class="subTitle">판매관리 > 상품수정</div>
</div>
		...
		<div class="imgUploadBox">
			...
			<div class="imgUploadInner">
				...
				<div class="imgInputBox">
					<ol type="1">
						<li>현재 상품수정 시 이미지는 새로 업로드해야 합니다.(수정예정)</li>
	...
	<div class="productInfoInputBox">
		...
		<div class="productInfoInputInner">
							...
							<div class="infoInputBox">
								...
								<div class="inputBoxInner">
									<div id="way_Box2" class="boxAlign">
										...
										<input class="addr1" type="text" id="productAddr" name="productAddr" disabled="disabled" value="${productDto.productAddr}"/>
									</div>
								</div>
								<div class="inputOkCheck">
							...
							<div class="infoInputBox">
								...
								<div class="inputBoxInner">
									...
									<div id="checkTitle" class="checkInput"></div>
										<input id="titleInput" type="text" onkeydown="checkTitleLimit();" onkeyup="checkTitleOk();" placeholder="제목을 입력해주세요.(10자 이상 30자 이하)" value="${productDto.productSubject}" />
									</div>
								</div>
								<div class="inputOkCheck">
									<img id="title_inputOkIMG" src="${pageContext.request.contextPath}/resources/img/join/after_all.png"/>
								</div>				
							</div>
							<div class="infoInputBox">
								...
								<div class="inputBoxInner">
									<div class="boxAlign3">
										<textarea id="contentInput" onkeyup="checkContent();" placeholder="상품설명을 입력해주세요.(10자 이상)">${productDto.productContext}</textarea>
									</div>
								</div>									
								<div class="inputOkCheck">
									<img id="content_inputOkIMG" src="${pageContext.request.contextPath}/resources/img/join/after_all.png"/>
								</div>				
							</div>
							<div class="infoInputBox">
								...
								<div class="inputBoxInner">
									...
									<div id="checkPrice" class="checkInput"></div>
										<input id="priceInput" type="text" onkeyup="checkPriceLimit();" onkeydown="return checkPriceLimit();" value="${productDto.productPrice}"/>
									</div>
								</div>									
								<div class="inputOkCheck">
									<img id="price_inputOkIMG" src="${pageContext.request.contextPath}/resources/img/join/after_all.png"/>
								</div>
							</div>
							<div class="infoInputBox">
								...
								<div class="inputBoxInner">
									...
									<div id="checkStock" class="checkInput"></div>
										<input id="stockInput" type="text" onkeyup="checkStockLimit();" onkeydown="return checkStockLimit();" value="${productDto.productStock}"/>
									</div>
								</div>				
								<div class="inputOkCheck">
									<img id="stock_inputOkIMG" src="${pageContext.request.contextPath}/resources/img/join/after_all.png"/>
								</div>				
							</div>
						</div>
					</td>
			...
			<div class="productInfoBottom">
				<div id="registerOk" class="productInfoBottomBtn">수정완료</div>
			</div>
		</div>
	</div>
<!-- 			<div class="testBox">준비중</div> -->
...
var contentValidity = true;
var priceValidity = true;
var stockValidity = true;
...
// 선택한 Thumbnail Img
var mainThumbnail = "";
var updateProductNo = ${productDto.productNo}; 
</script>